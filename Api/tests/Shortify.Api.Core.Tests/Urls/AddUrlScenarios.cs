...
public class AddUrlScenarios
{
	...
	[Fact]
	public async Task Should_save_shortened_url()
	{
		var request = CreateAddUrlRequest();
		var response = await _handler.HandleAsync(request, default);
	...
	[Fact]
	public async Task Should_save_shortened_url_with_created_by_and_created_on()
	{
		var request = CreateAddUrlRequest();
		var response = await _handler.HandleAsync(request, default);
	...
	private static AddUrlRequest CreateAddUrlRequest(string createdBy = "Cj@gmail.com")
	{
		return new AddUrlRequest(new Uri("https://chathuraliyanage.info"), createdBy);
	}
}